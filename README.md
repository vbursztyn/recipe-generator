...
Most ingredient substitution lists are stored in keywords/transforms.py. 

In the case of "to healthy/unhealthy", there is an initial strategy that loops through ingredients that are similar to the ones found in the recipe (according to ingredient_data/similar_ingredients.pickle, generated by calculate_similar_ingredients.py) and consults USDA nutritional data in search of alternatives with a healthier nutrient profile (according to ingredient_data/ingredient_nutrients.pickle, generated by get_ingredient_nutrients.py). This first strategy is then complemented with substitution lists stored in keywords/transforms.py, like all other cases below. 
